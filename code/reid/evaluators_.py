...
def pairwise_distance(features_gal, features_query, gallery=None, metric=None):

    x = features_query
    y = torch.cat([features_gal[f].unsqueeze(0) for f, _, _, _ in gallery], 0)
...
def pairwise_distance_0(features, query=None, gallery=None, metric=None):
    ...
    if top1:
      ...
      if not dataset:
        cmc_configs = {
            ...
            'shelf': dict(separate_camera_set=False,  #xiugai
                               single_gallery_shot=False,
                               first_match_break=True)}
        ...
        print('CMC Scores{:>12}{:>12}{:>12}{:>12}'
              .format('allshots', 'cuhk03', 'market1501', 'shelf'))
        for k in cmc_topk:
            print('  top-{:<4}{:12.1%}{:12.1%}{:12.1%}{:12.1%}'
                  .format(k, cmc_scores['allshots'][k - 1],
                          cmc_scores['cuhk03'][k - 1],
                          cmc_scores['market1501'][k - 1],
                          cmc_scores['shelf'][k - 1]))
        ...
        if (dataset == 'cuhk03'):
          ...
          cmc_configs = {
              'shelf': dict(separate_camera_set=False,
                                 single_gallery_shot=False,
                                 first_match_break=True)
                      }
          cmc_scores = {name: cmc(distmat, query_ids, gallery_ids,
                                  query_cams, gallery_cams, **params)
                        for name, params in cmc_configs.items()}
          print('CMC Scores{:>12}'.format('shelf'))
          for k in cmc_topk:
              print('  top-{:<4}{:12.1%}'
                    .format(k,
                            cmc_scores['shelf'][k-1]))
          return cmc_scores['shelf'][0], mAP
