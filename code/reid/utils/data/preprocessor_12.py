...
class Preprocessor(object):
    def __init__(self, dataset, root=None, with_pose=False, pose_root=None, pid_imgs=None, height=256, width=128, pose_aug='no', transform=None, img_=None):
    ...
    def __getitem__(self, indices):
        if isinstance(indices, (tuple, list)):
            if not self.with_pose:
                return [self._get_single_item(index, self.img_) for index in indices]
            else:
    ...
    def _get_single_item(self, index):
        fname, pid, camid, id= self.dataset[index]
        # fpath = fname
        # if self.root is not None:
        #     fpath = osp.join(self.root, fname)
        # img = Image.open(fpath).convert('RGB')
        img = self.transform(self.img_[id])
    ...
    def _get_single_item_with_pose(self, index):
        ...
        pid_query = list(self.pid_imgs[pid])   #xiugai
        if fname in pid_query and len(pid_query)>1:
            pid_query.remove(fname)
        pname = osp.splitext(pid_query)[0]
